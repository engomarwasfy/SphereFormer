...
class AttentionStep2(Function):
    ...
    def forward(ctx, attn, v, index0, index0_offsets, index1, index1_offsets, n_max):
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(f"attn_step2: forward time: {time.time() - t}s, M: {M}, h: {h}")
    ...
    def backward(ctx, grad_output):
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(f"attn_step2: backward time: {time.time() - t}s, M: {M}, h: {h}")
...
class DotProdWithIdx(Function):
    ...
    def backward(ctx, grad_output):
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(f"dot_prod_with_idx: backward time: {time.time() - t}s, M: {M}")
...
class DotProdWithIdxAll(Function):
    ...
    def forward(ctx, q, index_q, index_q_offsets, k, index_k, index_k_offsets, table_q, table_k, rel_idx, n_max):
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(f"dot_prod_with_idx_all: forward time: {time.time() - t}s, M: {M}")
    ...
    def backward(ctx, grad_output):
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(f"dot_prod_with_idx_all: backward time: {time.time() - t}s, M: {M}")
...
class AttentionStep2WithRelPosValue(Function):
    ...
    def forward(ctx, attn, v, index0, index0_offsets, n_max, index1, index1_offsets, table, rel_idx):
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(
                f"attn_step2_with_rel_pos_value: forward time: {time.time() - t}s, M: {M}, h: {h}"
            )
    ...
    def backward(ctx, grad_output):
        ...
        if DEBUG:
            torch.cuda.synchronize()
            print(
                f"attn_step2_with_rel_pos_value: backward time: {time.time() - t}s, M: {M}, h: {h}"
            )
